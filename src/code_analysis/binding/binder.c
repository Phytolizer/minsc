...
static BindUnaryOperatorKindResult bind_unary_operator_kind(
        SyntaxKind kind, ObjectType operand_type);
...
static BindBinaryOperatorKindResult bind_binary_operator_kind(
        SyntaxKind kind, ObjectType left_type, ObjectType right_type);

static BoundExpression* bind_literal_expression(
        Binder* binder, LiteralExpressionSyntax* syntax);
static BoundExpression* bind_unary_expression(Binder* binder,
                                              UnaryExpressionSyntax* syntax);
static BoundExpression* bind_binary_expression(Binder* binder,
                                               BinaryExpressionSyntax* syntax);
static BoundExpression* bind_parenthesized_expression(
        Binder* binder, ParenthesizedExpressionSyntax* syntax);
...
BoundExpression* binder_bind_expression(Binder* binder,
    ...
    switch (expression_syntax_kind(syntax)) {
        case SYNTAX_KIND_LITERAL_EXPRESSION:
            return bind_literal_expression(binder,
        ...
        case SYNTAX_KIND_BINARY_EXPRESSION:
            return bind_binary_expression(binder,
                                          (BinaryExpressionSyntax*)syntax);
        case SYNTAX_KIND_PARENTHESIZED_EXPRESSION:
            return bind_parenthesized_expression(
                    binder, (ParenthesizedExpressionSyntax*)syntax);
        default:
...
static BindUnaryOperatorKindResult bind_unary_operator_kind(
    ...
    switch (kind) {
        case SYNTAX_KIND_PLUS_TOKEN:
            return (BindUnaryOperatorKindResult)SUM_JUST(
...
static BindBinaryOperatorKindResult bind_binary_operator_kind(
    ...
    switch (kind) {
        case SYNTAX_KIND_PLUS_TOKEN:
            return (BindBinaryOperatorKindResult)SUM_JUST(
...
static BoundExpression* bind_literal_expression(
        Binder* binder, LiteralExpressionSyntax* syntax) {
    (void)binder;
    return bound_literal_expression_new(
            object_dup(syntax->literal_token->value));
}
...
static BoundExpression* bind_unary_expression(Binder* binder,
    ...
    BindUnaryOperatorKindResult operator_kind = bind_unary_operator_kind(
            syntax->operator_token->kind, bound_expression_type(operand));
    if (!operator_kind.present) {
        BUF_PUSH(&binder->diagnostics,
                 str_printf("Unary operator '" str_fmt "' is not "
                            "defined for type '" str_fmt "'",
                            str_arg(syntax->operator_token->text),
                            str_arg(object_type_string(
...
static BoundExpression* bind_binary_expression(Binder* binder,
    ...
    BindBinaryOperatorKindResult operator_kind =
            bind_binary_operator_kind(syntax->operator_token->kind,
                                      bound_expression_type(left),
                                      bound_expression_type(right));
    if (!operator_kind.present) {
        BUF_PUSH(&binder->diagnostics,
                 str_printf("Binary operator '" str_fmt "' is not "
